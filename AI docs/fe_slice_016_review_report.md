...
- **Searchable Country Code Phone Input (`PhoneInput`)**:
  - Created a custom reusable component featuring a searchable country code dropdown (fetched from `countries-list` library locally).
...
- **Stacking Order Fix (Z-Index blocking)**:
  ...
  - Applied `relative z-20` and `relative z-30` permanent stacking z-indexes to the form cards, so that dropdowns always paint on top of subsequent card sections.
- **Simulated PDF MoU Reader**:
  - Added a "Lihat PDF" trigger inside the MoU Card on the organization detail page (`/extracurriculars-admin/[id]`).
  - Launches a premium simulated PDF document reader modal displaying a beautifully formatted official MoU letterhead, pasal/clauses, dynamic representative names, and emerald digital verification signatures/stamps.
...
*   [page.tsx](file:///C:/Users/yogaa/Document%20Local/SAKODE/WEB%20DEVELOP/sakode-academy/app/%28admin%29/extracurriculars-admin/%5Bid%5D/page.tsx) — Details page with new Simulated PDF MoU reader.
...
3. [ ] Click on the newly registered school to view details:
   ...
   * Locate the MoU card on the right-hand panel.
   * Click **Lihat PDF** to open the custom PDF MoU Viewer. Verify the letterhead, school and PIC names match the school's data, and closing digital materai/stamps render beautifully.
