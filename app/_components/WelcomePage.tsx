...
export function WelcomePage() {
	...
	return (
		<div className='relative min-h-screen w-full flex flex-col justify-between items-center bg-background text-foreground overflow-hidden font-sans transition-colors duration-300'>
					...
					<UI.Card accentColor="yellow">
						<div className='gap-6 p-4 sm:p-8 flex flex-col'>
							...
							<div className='flex flex-col gap-2'>
								<div className='flex justify-between items-center text-sm font-bold text-zinc-550 dark:text-zinc-450'>
									<span>Tahap Pengembangan Sistem</span>
									<span className='text-sakode-yellow font-black'>2%</span>
								</div>
								...
								>
									<div
										className={
											selectedStyle === "claymorphism"
												? "h-full w-[2%] shadow-[inset_-2px_-2px_4px_rgba(0,0,0,0.15),_inset_2px_2px_4px_rgba(255,255,255,0.4)] rounded-full bg-sakode-yellow"
												: selectedStyle === "neobrutalism"
												? "h-full w-[2%] bg-sakode-yellow border-r-2 border-zinc-900 dark:border-white"
												: selectedStyle === "glassmorphism" || selectedStyle === "liquid-glass"
												? "h-full w-[2%] rounded-full bg-linear-to-r from-sakode-pink to-sakode-orange border-r border-white/30"
												: selectedStyle === "bento-grid"
												? "h-full w-[2%] rounded-r-md bg-sakode-yellow"
												: selectedStyle === "minimalism"
												? "h-full w-[2%] bg-zinc-900 dark:bg-white"
												: "h-full w-[2%] rounded-full bg-sakode-yellow"
										}
									/>
								</div>
							</div>
						...
						{comingFeatures.map((feat, index) => {
							...
							return (
								<Tooltip key={index}>
									...
									<Tooltip.Content className='bg-zinc-900 text-zinc-350 border border-zinc-850 text-xs rounded-lg px-3 py-1.5 shadow-xl z-50'>
			...
			{/* Footer */}
			<footer className='relative w-full max-w-6xl mx-auto py-8 px-4 flex flex-col sm:flex-row justify-between items-center gap-4 text-xs sm:text-sm text-zinc-400 dark:text-zinc-550 border-t border-zinc-200/50 dark:border-zinc-900/60 z-10 font-medium'>
