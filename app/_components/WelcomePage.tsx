...
import { Button, Input, Tooltip, Card, ProgressBar } from "@heroui/react";
import { subscribeEmail } from "../_actions/subscribe";
...
export function WelcomePage() {
  ...
  const handleSubscribe = async (e: SyntheticEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!email) return;
    ...
    try {
      ...
      if (result.success) {
        ...
        setErrorMessage(result.error || "Gagal menyimpan email.");
      }
    } catch (err) {
      setIsLoading(false);
  ...
  const comingFeatures = [
    {
      ...
      desc: "Registrasi murid baru dengan alur sistematis untuk kelas pemrograman intensif.",
    ...
    {
      number: "02",
      title: "Pendaftaran Trial Gratis",
      desc: "Sesi uji coba belajar pemrograman untuk menguji materi dan metode pembelajaran.",
    ...
    },
    {
      ...
      desc: "Materi dan kurikulum terstruktur dari tingkat dasar hingga standar industri.",
    ...
    },
    {
      ...
      desc: "Informasi portofolio mentor profesional industri IT dengan alokasi plotting yang tepat.",
    ...
    },
    {
      ...
      desc: "Sistem penjadwalan kelas mentoring 1-on-1 langsung bersama praktisi industri.",
    ...
    <div className="relative min-h-screen w-full flex flex-col justify-between items-center bg-background text-foreground overflow-hidden font-sans transition-colors duration-300">
      ...
      <main className="relative flex-1 w-full max-w-6xl flex flex-col items-center justify-center px-4 sm:px-6 lg:px-8 py-12 sm:py-16 z-10">
        ...
        <div className="text-center max-w-3xl mb-12 sm:mb-16">
          ...
          >
            Sistem Akademik<br />
            <span className="text-sakode-yellow">
              Sakode Academy
            </span>
          </motion.h1>
          ...
          <Card className="bg-white/80 dark:bg-zinc-950/40 backdrop-blur-md border border-zinc-200 dark:border-zinc-800/80 shadow-lg dark:shadow-2xl p-2 rounded-3xl overflow-hidden">
            <Card.Content className="gap-6 p-6 sm:p-10 flex flex-col">
              ...
              <div className="flex flex-col gap-2">
                ...
                <ProgressBar value={2} aria-label="Development progress" className="w-full">
              ...
              <div className="grid grid-cols-4 gap-3 sm:gap-4 py-4 text-center">
                {[
                  ...
                  <div key={idx} className="bg-zinc-50 dark:bg-zinc-900/60 border border-zinc-200/50 dark:border-zinc-800/50 rounded-2xl p-3 sm:p-4 shadow-xs">
              ...
              {/* Newsletter / Notify Me Form */}
              <div className="mt-2 border-t border-zinc-200/50 dark:border-zinc-800/50 pt-6 flex flex-col gap-3">
                {!isSubmitted ? (
                  <>
                    <form
                    ...
                    >
                      <Input
                        ...
                        className="flex-1 bg-white dark:bg-zinc-900/70 border border-zinc-200 dark:border-zinc-800 hover:border-zinc-300 dark:hover:border-zinc-700 focus:border-zinc-400 dark:focus:border-zinc-500 focus:outline-none transition-colors h-12 rounded-xl text-zinc-900 dark:text-zinc-200 placeholder-zinc-400 dark:placeholder-zinc-500 font-medium px-4 w-full"
                      />
                      <Button
                        type="submit"
                        isDisabled={isLoading}
                        className="bg-linear-to-r from-sakode-pink to-sakode-orange hover:opacity-90 font-bold px-6 h-12 rounded-xl text-white shadow-md active:scale-[0.98] transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center min-w-42.5 cursor-pointer"
                      >
                        {isLoading ? (
                    ...
                    {errorMessage && (
                      <p className="text-xs text-sakode-red font-semibold pl-2 text-left">
                        {errorMessage}
                      </p>
                    )}
                  </>
                  ...
                  <motion.div
                    ...
                    animate={{ opacity: 1, y: 0 }}
                    className="flex flex-col items-center text-center p-4 bg-emerald-500/10 dark:bg-emerald-950/20 border border-emerald-500/20 dark:border-emerald-800/30 rounded-2xl gap-2"
                  >
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-bold text-lg">
                      ✓
                    </div>
                    <div>
                      <h4 className="font-bold text-zinc-800 dark:text-zinc-100">Pendaftaran Berhasil!</h4>
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">Kami akan mengabari Anda segera setelah platform resmi diluncurkan.</p>
                    </div>
                  </motion.div>
                )}
              </div>
        ...
        <div className="w-full">
          ...
          >
            {comingFeatures.map((feat, index) => (
              <Tooltip key={index}>
                <Tooltip.Trigger>
                  <div className={`group flex flex-col p-6 rounded-2xl border transition-all duration-300 hover:scale-[1.02] cursor-help w-full h-full text-left
