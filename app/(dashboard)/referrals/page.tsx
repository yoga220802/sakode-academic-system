...
export default function ReferralManagementPage() {
  ...
  const [simulationState, setSimulationState] = useState<
    "default" | "loading" | "empty" | "error" | "expired" | "code-exhausted" | "no-attribution" | "corrected-attribution"
  >("default");
  ...
  // Selection states
  const [selectedProgramId, setSelectedProgramId] = useState<string | null>("REF-PRG-001");
  const [copiedCode, setCopiedCode] = useState<string | null>(null);

  // Modals & form states
  const [isCreateProgramOpen, setIsCreateProgramOpen] = useState(false);
  const [isGenerateCodeOpen, setIsGenerateCodeOpen] = useState(false);
  const [isCorrectionOpen, setIsCorrectionOpen] = useState<{ conversion: ReferralConversion } | null>(null);
  const [isStatusConfirmOpen, setIsStatusConfirmOpen] = useState<{ type: "activate" | "deactivate"; program: ReferralProgram } | null>(null);
  ...
  useEffect(() => {
    ...
    const basePrograms: ReferralProgram[] = [
      {
        ...
        desc: "Insentif tracking untuk alumni yang merekomendasikan Bootcamp SAKODE kepada kenalan atau publik.",
        status: "active",
        targetProgram: "Semua Program Bootcamp",
      ...
      },
      {
        ...
        desc: "Tracking program magang dan rujukan akademis universitas mitra program digital.",
        status: "active",
      ...
      },
      {
        ...
        desc: "Kampanye promosi media sosial influencer rujukan kurikulum pemrograman dasar.",
        status: "active",
      ...
      },
      {
        ...
        desc: "Tracking internal rekomendasi siswa khusus oleh mentor active semester ganjil.",
        status: "expired",
    ...
    const baseCodes: ReferralCode[] = [
      { code: "AKBAR26", referrer: "Akbar Ramadhan", status: "active", used: 15, limit: 50, dateCreated: "01 Mar 2026" },
      { code: "MANDIRIIT", referrer: "Magang Mandiri", status: "active", used: 6, limit: "unlimited", dateCreated: "15 Mar 2026" },
      { code: "YOGA22", referrer: "Yoga Pratama", status: "active", used: 12, limit: 20, dateCreated: "02 Feb 2026" },
      { code: "EXHAUST10", referrer: "Promo Event Habis", status: "exhausted", used: 50, limit: 50, dateCreated: "10 Jan 2026" }
    ];

    // Basic conversions
    const baseConversions: ReferralConversion[] = [
      { id: "CONV-001", studentName: "Dzulkifli Putra", email: "dzulkifli.putra@gmail.com", codeUsed: "AKBAR26", dateConverted: "13 Mar 2026 16:30", registrationStatus: "pending" },
      { id: "CONV-002", studentName: "Endah Lestari", email: "endah.lestari@yahoo.com", codeUsed: "MANDIRIIT", dateConverted: "13 Mar 2026 14:15", registrationStatus: "pending" },
      { id: "CONV-003", studentName: "Rian Hidayat", email: "rian.hidayat@outlook.com", codeUsed: "AKBAR26", dateConverted: "12 Mar 2026 18:22", registrationStatus: "pending" },
      { id: "CONV-004", studentName: "Siti Aminah", email: "siti.aminah@gmail.com", codeUsed: "AKBAR26", dateConverted: "11 Mar 2026 10:05", registrationStatus: "approved" },
      { id: "CONV-005", studentName: "Joko Susilo", email: "joko.susilo@hotmail.com", codeUsed: "YOGA22", dateConverted: "10 Mar 2026 09:12", registrationStatus: "rejected" }
    ];
    ...
    if (simulationState === "loading") {
      ...
      setCodes(baseCodes.map(c => ({ ...c, status: "exhausted", used: typeof c.limit === "number" ? c.limit : 99 })));
      setConversions(baseConversions);
      ...
      setPrograms(basePrograms);
      setCodes(baseCodes);
      setConversions(baseConversions.map(c => c.id === "CONV-003" ? { ...c, codeUsed: "MANDIRIIT", studentName: "Rian Hidayat (Koreksi Rujukan)" } : c));
    } else {
      setPrograms(basePrograms);
      setCodes(baseCodes);
      setConversions(baseConversions);
  ...
  const filteredPrograms = useMemo(() => {
    ...
    return programs.filter(item => {
      ...
      const matchesTarget = selectedTargetFilter === "all" || item.targetProgram === selectedTargetFilter;
      const matchesStatus = selectedStatusFilter === "all" || item.status === selectedStatusFilter;
      return matchesSearch && matchesTarget && matchesStatus;
    });
  }, [programs, searchQuery, selectedTargetFilter, selectedStatusFilter, simulationState]);
  ...
  const handleCreateProgram = (e: React.FormEvent) => {
    ...
    const newPrg: ReferralProgram = {
      ...
      startDate: new Date(newProgramForm.startDate).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }),
      endDate: new Date(newProgramForm.endDate).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" }),
      codeCount: 0,
      conversionCount: 0
    };
  ...
  const handleGenerateCode = (e: React.FormEvent) => {
    ...
    const newC: ReferralCode = {
      ...
      status: "active",
      used: 0,
      limit: newCodeForm.limit === "unlimited" ? "unlimited" : newCodeForm.limitValue,
      dateCreated: new Date().toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric" })
    };
    ...
    if (selectedProgramId) {
      setPrograms(prev => prev.map(p => p.id === selectedProgramId ? { ...p, codeCount: p.codeCount + 1 } : p));
    }
  ...
  const handleCorrection = (e: React.FormEvent) => {
    ...
    setConversions(prev => prev.map(c => c.id === targetConv.id ? { ...c, codeUsed: newCode } : c));
  ...
  const handleShareLink = (code: string) => {
    ...
    if (navigator.share) {
      ...
      setToastMessage({
        type: "success",
        text: "Fitur share native tidak didukung browser. Link canonical berhasil disalin!"
      });
    }
  };
  ...
  const getSubElementClass = (type: "divider" | "inner-card" | "timeline-dot" | "timeline-line") => {
    switch (selectedStyle) {
      ...
      case "claymorphism":
        ...
        if (type === "inner-card") return "p-3.5 bg-slate-50 dark:bg-zinc-900/50 border border-slate-200/20 dark:border-zinc-700/20 rounded-2xl shadow-[inset_-2px_-2px_4px_rgba(0,0,0,0.05),_inset_2px_2px_4px_rgba(255,255,255,0.05)]";
        return "";
  ...
  const getCloseButtonClass = () => {
    ...
    switch (selectedStyle) {
      ...
      case "minimalism":
        return base + "hover:bg-zinc-100 dark:hover:bg-zinc-900 rounded-sm text-zinc-400 dark:text-zinc-500";
      case "bento-grid":
      case "sakode-modern":
      default:
  ...
  return (
    <div className="flex flex-col gap-10 w-full text-left font-sans pb-16">
      ...
      <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-1.5 items-center bg-zinc-100/80 dark:bg-zinc-805 p-1.5 rounded-xl border border-zinc-200/50 dark:border-zinc-800/50 z-20">
          ...
          {[
            ...
            <button
              key={state.id}
              onClick={() => { setSimulationState(state.id); setSelectedProgramId(state.id === "empty" ? null : "REF-PRG-001"); }}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: "Total Klik Rujukan", value: metrics.clicks, color: selectedColor },
          { label: "Kode Referral Aktif", value: metrics.activeCodes, color: "purple" as const },
          { label: "Pendaftar Terkonversi", value: metrics.conversions, color: "green" as const },
          { label: "Program Berjalan", value: metrics.activePrograms, color: "orange" as const }
        ].map((m, idx) => (
          <UI.Card key={idx} accentColor={m.color}>
            <div className="p-4 flex flex-col gap-1.5 justify-start text-left relative overflow-hidden">
              ...
              <span className="text-2xl font-bold text-zinc-900 dark:text-white leading-none mt-1">
                {simulationState === "loading" ? "..." : m.value.toLocaleString("id-ID")}
              </span>
            </div>
          </UI.Card>
        ))}
            ...
            <div className="p-5 flex flex-col gap-5">
              ...
              {simulationState === "loading" ? (
                ...
                <div className="flex flex-col gap-3 py-4">
                  {[1, 2, 3].map((s) => (
                    <div key={s} className="flex flex-col gap-2 p-4 border border-zinc-150 dark:border-zinc-800 rounded-2xl animate-pulse">
                ...
                <div className="flex flex-col items-center justify-center py-10 px-4 text-center">
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-xs mb-4">
                    Sistem gagal sinkronisasi ke server. Silakan muat ulang atau coba beberapa saat lagi.
                  </p>
                  <UI.Button 
                    variant="secondary" 
                    accentColor="red" 
                ...
                <div className="flex flex-col items-center justify-center py-14 px-4 text-center">
                  ...
                  <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-xs">
                    Belum ada program referral terdaftar, atau kata kunci pencarian Anda tidak cocok.
                  </p>
                </div>
              ) : (
                // 3.4 Loaded list
                      ...
                      >
                        ...
                        <div className="flex justify-between items-start gap-3">
                          ...
                          <div className="shrink-0">
                            {prg.status === "active" ? (
                              <UI.Badge variant="success" accentColor="green">Aktif</UI.Badge>
                            ) : prg.status === "expired" ? (
                              <UI.Badge variant="default">Expired</UI.Badge>
                            ) : (
                              <UI.Badge variant="warning" accentColor="orange">Nonaktif</UI.Badge>
                            )}
                          </div>
                        </div>
          ...
          <AnimatePresence mode="wait">
            {!selectedProgram ? (
                  ...
                  <div className="p-8 text-center flex flex-col items-center justify-center gap-4 py-24">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-xs text-zinc-500 dark:text-zinc-400 max-w-xs mt-1">
                        Pilih salah satu kampanye referral di sebelah kiri untuk melihat daftar inventori kode rujukan, link canonical, limits, serta data pendaftar teratribusi.
                      </p>
                    </div>
                  </div>
                </UI.Card>
              ...
              <motion.div
                ...
                className="w-full"
              >
                <UI.Card accentColor={selectedProgram.status === "active" ? selectedColor : "orange"}>
                  <div className="p-5 flex flex-col gap-5 text-left relative">
                      ...
                      <div className="flex gap-2 mt-2">
                        {selectedProgram.status === "active" ? (
                          <UI.Button
                            variant="secondary"
                            accentColor="red"
                            onClick={() => setIsStatusConfirmOpen({ type: "deactivate", program: selectedProgram })}
                          ...
                          <UI.Button
                            variant="primary"
                            accentColor="green"
                            onClick={() => setIsStatusConfirmOpen({ type: "activate", program: selectedProgram })}
                    ...
                    <div className="flex flex-col gap-3">
                      ...
                      <div className="flex flex-col gap-2.5">
                        {codes.map((c, cIdx) => {
                          const isExhausted = c.status === "exhausted" || (typeof c.limit === "number" && c.used >= c.limit);
                          return (
                            <div
                              key={cIdx}
                              className={getSubElementClass("inner-card")}
                            >
                              <div className="flex flex-col gap-2">
                                ...
                                <div className="flex justify-between items-center gap-2">
                                  <div className="flex items-center gap-2">
                                    ...
                                    {isExhausted ? (
                                      <UI.Badge variant="accent" accentColor="red" className="text-[8.5px]! font-medium! px-1.5! py-0!">
                                        Habis
                                      </UI.Badge>
                                    ) : c.status === "suspended" ? (
                                      <UI.Badge variant="warning" accentColor="orange" className="text-[8.5px]! font-medium! px-1.5! py-0!">
                                        Suspended
                                      </UI.Badge>
                                    ) : (
                                      <UI.Badge variant="success" accentColor="green" className="text-[8.5px]! font-medium! px-1.5! py-0!">
                                        Aktif
                                      </UI.Badge>
                                    )}
                                  </div>
                                ...
                                <div className="text-[10.5px] text-zinc-500 dark:text-zinc-300">
                                  Referrer: <strong className="font-bold text-zinc-800 dark:text-zinc-100">{c.referrer}</strong>
                                </div>
                            ...
                            >
                              <div className="flex justify-between items-start gap-2">
                                ...
                                <div className="flex flex-col items-end gap-1.5 shrink-0">
                                  {conv.registrationStatus === "pending" ? (
                                    <UI.Badge variant="warning" accentColor="orange" className="text-[8.5px]! py-0 px-1.5!">Review</UI.Badge>
                                  ) : conv.registrationStatus === "approved" ? (
                                    <UI.Badge variant="success" accentColor="green" className="text-[8.5px]! py-0 px-1.5!">Disetujui</UI.Badge>
                                  ) : (
                                    <UI.Badge variant="accent" accentColor="red" className="text-[8.5px]! py-0 px-1.5!">Ditolak</UI.Badge>
                                  )}
                                  
                                  <span className="text-[9px] text-zinc-400 font-medium">
                                    {conv.dateConverted.split(" ")[0]} {conv.dateConverted.split(" ")[1]}
                                  </span>
                                </div>
                              </div>
                              ...
                              <div className="flex justify-between items-center border-t border-dashed border-zinc-200/60 dark:border-zinc-800/60 pt-2 text-[10.5px]">
                                <span className="text-zinc-500 dark:text-zinc-300 font-medium">
                                  Atribusi Kode: <strong className="font-mono text-zinc-800 dark:text-white">{conv.codeUsed}</strong>
                                </span>
      ...
      <AnimatePresence>
        {isCreateProgramOpen && (
          <div className="fixed inset-0 bg-black/45 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            ...
            >
              <UI.Card accentColor={selectedColor}>
                <form onSubmit={handleCreateProgram} className="flex flex-col gap-4 text-left">
                  ...
                  <div className="flex flex-col gap-3 text-xs">
                    <div className="flex flex-col gap-1">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Nama Kampanye Program</label>
                      <UI.Input
                        type="text"
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Deskripsi Kampanye</label>
                      <textarea
                        placeholder="Tulis tujuan kampanye, partner target, dll..."
                        value={newProgramForm.desc}
                        onChange={(e) => setNewProgramForm(prev => ({ ...prev, desc: e.target.value }))}
                        required
                    ...
                    <div className="flex flex-col gap-1">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Program Bootcamp Sasaran</label>
                      <div className="relative">
                        <UI.Select
                          value={newProgramForm.targetProgram}
                          onChange={(e) => setNewProgramForm(prev => ({ ...prev, targetProgram: e.target.value }))}
                          accentColor={selectedColor}
                          className="text-xs! py-2! pr-8! pl-3!"
                        >
                          <option value="Semua Program Bootcamp">Semua Program Bootcamp</option>
                          <option value="React & Next.js Professional">React & Next.js Professional</option>
                          <option value="Backend Dev Go/Docker">Backend Dev Go/Docker</option>
                          <option value="TypeScript & Data Structures">TypeScript & Data Structures</option>
                        </UI.Select>
      ...
      <AnimatePresence>
        {isGenerateCodeOpen && (
          <div className="fixed inset-0 bg-black/45 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            ...
            >
              <UI.Card accentColor={selectedColor}>
                <form onSubmit={handleGenerateCode} className="flex flex-col gap-4 text-left">
                  <div className="flex items-center justify-between border-b border-zinc-150 dark:border-zinc-800 pb-3">
                  ...
                  <div className="flex flex-col gap-3.5 text-xs">
                    <div className="flex flex-col gap-1">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Kode Rujukan (Alphanumeric, URL Safe)</label>
                      <UI.Input
                        type="text"
                      ...
                      <span className="text-[9px] text-zinc-400 font-medium block">
                        * Kode rujukan harus URL-safe (tidak mengandung spasi, karakter khusus, atau data PII).
                      </span>
                    </div>

                    <div className="flex flex-col gap-1">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Nama Pemilik / Sumber Rujukan</label>
                      <UI.Input
                        type="text"
                        placeholder="contoh: Ahmad Panjul (Alumni)"
                        value={newCodeForm.referrer}
                        onChange={(e) => setNewCodeForm(prev => ({ ...prev, referrer: e.target.value }))}
                        accentColor={selectedColor}
                        required
                        className="text-xs! py-2!"
                      />
                    ...
                    <div className="flex flex-col gap-2">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300 block">Batas Jumlah Penggunaan</label>
                      <div className="flex items-center gap-4 mt-1">
                        <label className="flex items-center gap-1.5 cursor-pointer font-medium">
                          <input
                            ...
                            onChange={() => setNewCodeForm(prev => ({ ...prev, limit: "unlimited" }))}
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer font-medium">
                          <input
                            ...
                            onChange={() => setNewCodeForm(prev => ({ ...prev, limit: "value" }))}
                            className="cursor-pointer"
                          />
                      ...
                      {newCodeForm.limit === "value" && (
                        <div className="mt-2.5 max-w-30">
                          <UI.Input
                            ...
                            onChange={(e) => setNewCodeForm(prev => ({ ...prev, limitValue: parseInt(e.target.value) || 1 }))}
                            accentColor={selectedColor}
      ...
      <AnimatePresence>
        {isCorrectionOpen && (
          <div className="fixed inset-0 bg-black/45 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            ...
            >
              <UI.Card accentColor={selectedColor}>
                <form onSubmit={handleCorrection} className="flex flex-col gap-4 text-left">
                  <div className="flex items-center justify-between border-b border-zinc-150 dark:border-zinc-800 pb-3">
                    <h3 className="text-xs font-bold uppercase tracking-wider text-zinc-555 dark:text-zinc-200">
                  ...
                  <div className="flex flex-col gap-3 text-xs leading-normal">
                    <p className="text-zinc-600 dark:text-zinc-300">
                      Koreksi rujukan manual dilakukan jika pendaftar salah mengisi kode referral atau lupa memasukkan parameter link saat mendaftar:
                    </p>

                    <div className={getSubElementClass("inner-card")}>
                      <span className="font-bold text-zinc-800 dark:text-zinc-100 block">Siswa: {isCorrectionOpen.conversion.studentName}</span>
                      <span className="text-[10px] text-zinc-500 block mt-0.5">Email: {isCorrectionOpen.conversion.email}</span>
                      <span className="text-[10px] text-zinc-500 block mt-0.5">Kode Rujukan Saat Ini: <strong className="font-mono text-zinc-800 dark:text-zinc-200">{isCorrectionOpen.conversion.codeUsed}</strong></span>
                    </div>

                    <div className="flex flex-col gap-1.5 mt-2">
                      <label className="font-bold text-zinc-700 dark:text-zinc-300">Pilih Kode Rujukan Pengganti</label>
                      <div className="relative">
                        <UI.Select
        ...
        {isStatusConfirmOpen && (
          <div className="fixed inset-0 bg-black/45 backdrop-blur-xs flex items-center justify-center p-4 z-50">
            <motion.div
              ...
              className="w-full max-w-md relative"
            >
              <UI.Card accentColor={isStatusConfirmOpen.type === "activate" ? "green" : "red"}>
                <div className="flex flex-col gap-4 text-left">
                  ...
                  <div className="flex flex-col gap-3 py-1 text-xs">
                    <p className="text-zinc-700 dark:text-zinc-200 font-medium leading-relaxed">
                      Apakah Anda yakin ingin {isStatusConfirmOpen.type === "activate" ? "mengaktifkan kembali" : "menonaktifkan sementara"} program referral:
                    </p>
                    <div className={getSubElementClass("inner-card")}>
                      <span className="font-bold text-zinc-850 dark:text-zinc-100 block">{isStatusConfirmOpen.program.name}</span>
                      <span className="text-[10px] text-zinc-450 dark:text-zinc-400 font-medium mt-0.5 block">{isStatusConfirmOpen.program.targetProgram}</span>
                    </div>
                    {isStatusConfirmOpen.type === "deactivate" ? (
                      <p className="text-[10px] text-rose-500/80 font-normal leading-normal italic">
                        * Menonaktifkan program akan menangguhkan seluruh tracking atribusi rujukan dari kode terkait untuk sementara.
                      </p>
                    ) : (
                      <p className="text-[10px] text-emerald-600/80 font-normal leading-normal italic">
                        * Mengaktifkan kembali program akan langsung mengaktifkan tracking atribusi rujukan untuk seluruh kode inventori aktif.
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex gap-2.5 justify-end mt-2 border-t border-zinc-150 dark:border-zinc-800 pt-4">
                    ...
                    <UI.Button 
                      ...
                      className="text-xs! py-2! font-semibold! cursor-pointer"
                    >
                      {isStatusConfirmOpen.type === "activate" ? "Aktifkan" : "Nonaktifkan"}
                    </UI.Button>
                  </div>
                </div>
              </UI.Card>
