...
export default function EditPrincipalMembershipPage({ params }: EditMembershipPageProps) {
  ...
  const targetMembership = useMemo(() => {
    return memberships.find((m) => m.id === targetId) || null;
  ...
  useEffect(() => {
    if (targetMembership) {
      setForm({
        principalName: targetMembership.principalName,
        principalEmail: targetMembership.principalEmail,
        status: targetMembership.status
      });
      setSelectedOrgs(targetMembership.assignedOrgs || []);
    }
  }, [targetMembership]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const emailExists = memberships.some(
      (m) => m.id !== targetId && m.principalEmail.toLowerCase() === form.principalEmail.toLowerCase()
    );
    if (emailExists) {
      setFormError("Alamat email ini sudah terdaftar sebagai Kepala Sekolah lain.");
      return;
    }
  ...
  if (!targetMembership) {
    return (
      <div className="py-16 text-center text-xs text-zinc-405 font-medium border border-dashed border-zinc-200 dark:border-zinc-800 rounded-2xl flex flex-col items-center gap-2">
        ...
        <span>Keanggotaan Kepala Sekolah tidak ditemukan.</span>
        <button
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-200 dark:border-zinc-800/60 pb-5">
        <div>
          ...
          <h1 className="text-xl md:text-2xl font-black text-zinc-900 dark:text-white leading-tight">
            Ubah Keanggotaan {targetMembership.principalName}
          </h1>
          <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-1">
            Perbarui data akun Kepala Sekolah atau sesuaikan daftar pemetaan sekolah mitra yang diizinkan untuk diakses.
          </p>
        </div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <span className="text-[9.5px] text-zinc-455 block truncate mt-0.5">
